...
import { Users, Truck, ShoppingBag, IndianRupee, ArrowRight, Clock } from 'lucide-react';
import { mockDashboardStats, mockOrders, mockDeliveryPartners } from '@/lib/mockData';
import Link from 'next/link';

export default function Dashboard() {
  const recentOrders = mockOrders.slice(0, 5);
  const onlinePartners = mockDeliveryPartners.filter((p) => p.status === 'online');
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <StatsCard
          ...
          change={mockDashboardStats.partnersGrowth}
          icon={Truck}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-6">
        ...
        <div className="card p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted">2nd Jan 2026</p>
            </div>
          </div>
          <div className="space-y-3">
            ...
            <div className="flex justify-between items-center pt-3 border-t border-border">
              ...
              <span className="font-bold text-lg text-primary">₹24,500</span>
            </div>
          </div>
        </div>
        ...
        <div className="card p-6">
          ...
          <div className="space-y-3">
            {[
              { status: 'pending', count: 4, color: 'bg-warning' },
              { status: 'confirmed', count: 8, color: 'bg-info' },
              { status: 'preparing', count: 12, color: 'bg-purple-500' },
              { status: 'out_for_delivery', count: 6, color: 'bg-primary' },
              { status: 'delivered', count: 38, color: 'bg-success' },
            ].map((item) => (
